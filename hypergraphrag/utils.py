...
def normalize_to_half_width(text: str) -> str:
    """将字符串中的全角字符转换为半角，并做 strip。
    全角空格（U+3000）→ 半角空格（U+0020）
    全角 ASCII（U+FF01～U+FF5E）→ 半角 ASCII（U+0021～U+007E）
    全角数字、字母同理。
    """
    if not text:
        return text
...
# ============================================================
# Phase 1 新增：chapter 嵌套解析
# ============================================================

def parse_chapter(chapter_str: str) -> dict:
    """
    解析 chapter 字段，返回结构化信息。
    ...
    """
    if not chapter_str or not chapter_str.strip():
        return {
            "chapter_level": 0,
            "parts": [],
            "parent": None,
...
def build_chapter_hierarchy(chapters: list[str]) -> dict:
    """
    将多个 chapter 字符串构建成树形结构。

    输入：["1", "1.2", "1.2.3", "1.3", "2", "2.1"]
    输出：{
        "1": {
            "children": {
                "2": {
...
def chapter_matches_query(chapter_str: str, query: str) -> bool:
    """
    判断某个 chapter 是否匹配查询条件。

    支持两种查询格式：
    - "1.2.3" 精确匹配
    - "1.2.*" 前缀匹配（* 表示该层级下所有子章节）

    输入：chapter_str="1.2.3", query="1.2"
    输出：True

    输入：chapter_str="1.2.3", query="1.2.*"
    输出：True

    输入：chapter_str="1.3.1", query="1.2.*"
    输出：False
    """
    parsed = parse_chapter(chapter_str)
    parts = parsed["parts"]

    if query.endswith(".*"):
        prefix = query[:-2].split(".")
        # 检查 parts 是否以 prefix 开头
        return len(parts) >= len(prefix) and all(
            parts[i] == prefix[i] for i in range(len(prefix))
        )
    else:
        q_parts = query.split(".")
        return parts == q_parts
