...
@retry(
    ...
    retry=retry_if_exception_type((RateLimitError, APIConnectionError, Timeout, asyncio.TimeoutError)),
)
async def minimax_complete_if_cache(
    ...
    **kwargs,
    ...
    """MiniMax 国内版 LLM（原生 API 调用）。

    默认使用 MiniMax M2.7 模型，需设置环境变量 MINIMAX_API_KEY。
    base_url 默认为国内版接口 https://api.minimax.chat/v1
    ...
    if not api_key:
        raise ValueError(
            "MiniMax api_key is not set. "
            "Set the MINIMAX_API_KEY environment variable or pass api_key explicitly."
        )
    ...
    # 强制 JSON 输出（MiniMax 支持 response_format 参数）
    payload["response_format"] = {"type": "json_object"}
    # 支持常见参数透传
    for k in ("temperature", "max_tokens", "top_p", "stop", "stream", "response_format"):
        if k in kwargs:
    ...
    logger.debug(f"MiniMax request URL: {url}")
    logger.debug(f"MiniMax request payload: {payload}")
        ...
        async with session.post(url, headers=headers, json=payload) as response:
            if response.status != 200:
                ...
                raise APIConnectionError(
                    f"MiniMax API returned status {response.status}: {text}"
                )
    ...
    # 兼容处理：MiniMax 返回 `` 思考块 + markdown fence + JSON
    # 1. 去掉 `` ... `` 纯文本思考块（无反引号包裹）
    content_for_json = content
    while "<think>" in content_for_json:
        start = content_for_json.find("<think>")
        end = content_for_json.find("</think>", start)
        if end == -1:
            break
        content_for_json = content_for_json[:start] + content_for_json[end + 4:]

    # 2. 去掉 markdown fence（```json ... ```）
    try:
        first_fence = content_for_json.find("```json")
        if first_fence == -1:
            first_fence = content_for_json.find("```")
        last_fence = content_for_json.rfind("```")
        if first_fence != -1 and last_fence != -1 and last_fence > first_fence:
            content_for_json = content_for_json[first_fence + (6 if "```json" in content_for_json[first_fence:first_fence+10] else 3):last_fence]
    except Exception:
        pass

    # 3. 去掉多余空白，取第一个 { 到最后一个 } 之间的内容
    content_for_json = content_for_json.strip()
    try:
        first_brace = content_for_json.find("{")
        last_brace = content_for_json.rfind("}")
        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
            json_candidate = content_for_json[first_brace : last_brace + 1]
            json.loads(json_candidate)  # 验证
...
class Model(BaseModel):
    ...
    Example usage:
        Model(gen_func=openai_complete_if_cache, kwargs={"model": "gpt-4", "api_key": os.environ.get("OPENAI_API_KEY", "your-key-here")})
...
class MultiModel:
    ...
    Usage example:
        ```python
        models = [
            Model(gen_func=openai_complete_if_cache, kwargs={"model": "gpt-4", "api_key": os.environ.get("OPENAI_API_KEY_1", "key1")}),
            Model(gen_func=openai_complete_if_cache, kwargs={"model": "gpt-4", "api_key": os.environ.get("OPENAI_API_KEY_2", "key2")}),
            Model(gen_func=openai_complete_if_cache, kwargs={"model": "gpt-4", "api_key": os.environ.get("OPENAI_API_KEY_3", "key3")}),
        ]
