...
async def extract_entities(
    ...
    async def _call_extract_prompt(chunk_key: str, content: str) -> dict:
        ...
        Phase 4 修复：不再用字符串拼接glean_result，而是逐个解析并合并 concept/relation。
        这样即使 glean_result 是 "no" 或空 JSON，也不会破坏主 JSON 的解析。
        """
        ...
        for glean_index in range(entity_extract_max_gleaning):
            ...
            # Phase 4 修复：逐个解析 glean_result，不再字符串拼接
            try:
                glean_cleaned = glean_result.strip()
                if glean_cleaned.startswith("```"):
                    lines = glean_cleaned.split("\n")
                    glean_cleaned = "\n".join(lines[1:-1] if lines[-1] == "```" else lines[1:])
                elif "```" in glean_cleaned:
                    first_fence = glean_cleaned.find("```")
    ...
    for cr in chunk_results:
        for concept in cr["concepts"]:
            ...
            name = normalize_to_half_width(raw_name)
            if not name:
                continue
            # 归一化后用 upper() 做去重 key（中文不变，仅全角→半角）
            name_key = name.upper()
            if name_key not in concept_map:
                concept_map[name_key] = {
                    "name": name,  # 存归一化后的 name（无尾部空格、无全角）
                    "domain_set": set(),
                    "description_list": [],
                    "source_chunks": set(),
                }
            concept_map[name_key]["domain_set"].update(
                normalize_to_half_width(d) for d in concept.get("domain", [])
            )
...
async def kg_query(
    ...
    try:
        ...
        # 兼容处理：MiniMax 有时返回纯数组而非 {"concepts": [...]} 格式
        if isinstance(kw_data, list):
            concepts_out = kw_data
            relations_out = []
        else:
        ...
        # 过滤空字符串
        ll_keywords = [k for k in ll_keywords if k]
        hl_keywords = [k for k in hl_keywords if k]
    except json.JSONDecodeError as e:
        print(f"JSON parsing error: {e} {final_result}")
        # JSON 解析失败时，用查询文本本身作为关键词继续
        ll_keywords = [query.strip()]
        hl_keywords = []

    # 所有检查通过，直接进入 RAG 构建上下文
...
async def _get_edge_data(
    ...
    """Phase 4 修复：relations_vdb 存储的是 src_id/tgt_id，而非 hyperedge_name。
    使用 get_edge(src_id, tgt_id) 代替 get_node(hyperedge_name)。"""
    results = await relations_vdb.query(keywords, top_k=query_param.top_k)

    if not len(results):
        return "", "", ""

    # Phase 4 修复：query() 只返回 id+distance，需要 get_by_ids 获取完整数据
    all_ids = [r["id"] for r in results]
    full_data_map = await relations_vdb.get_by_ids(all_ids)

    # Phase 4 修复：用 get_edge(src_id, tgt_id) 替代不存在的 get_node(hyperedge_name)
    edge_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_edge(full_data_map[r["id"]]["src_id"], full_data_map[r["id"]]["tgt_id"])
          for r in results]
    )
    ...
    # Phase 4 修复：用 src_id|tgt_id 作为 hyperedge 标识符
    edge_datas = [
        {"hyperedge": f"{k['src_id']}|{k['tgt_id']}", "rank": k["distance"], **v}
    ...
    # Phase 4 修复：get_node_edges 的参数是节点名，不是 hyperedge 标识符
    # 对于 relation 边，related_nodes 应该是边的端点节点
    all_related_nodes = await asyncio.gather(
        *[knowledge_graph_inst.get_node_edges(edge["hyperedge"].split("|")[0])
